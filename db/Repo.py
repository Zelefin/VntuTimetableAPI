...
class Repo:
    ...
    async def get_faculty(self, faculty_id) -> Faculty | None:
        return (await self.session.execute(select(Faculty).where(Faculty.id == faculty_id))).scalar_one_or_none()
    ...
    async def get_teacher_by_id(self, teacher_id: int) -> Teacher | None:
        return (await self.session.execute(select(Teacher).where(Teacher.id == teacher_id))).scalar_one_or_none()

    async def get_teacher_by_name(self, teacher_name: str) -> Teacher | None:
        return (await self.session.execute(select(Teacher).where(Teacher.name == teacher_name))).scalar()
